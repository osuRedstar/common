...
def readableMods(__mods):
	...
	#NC 추가
	#576 = DT, NC
	if __mods == 576 or  __mods & mods.NIGHTCORE > 0:
		r = r.replace('DT', 'NC')
		#r += "NC, "
	...
	if __mods & mods.KEY5 > 0:
		r += "K5"
	if __mods & mods.KEY6 > 0:
		r += "K6"
	if __mods & mods.KEY7 > 0:
		r += "K7"
	if __mods & mods.KEY8 > 0:
		r += "K8"
	#?
	if __mods & mods.KEYMOD > 0:
		r += "KEYMOD"
	if __mods & mods.FADEIN > 0:
		r += "FI"
	if __mods & mods.RANDOM > 0:
		r += "RD"
	#?
	if __mods & mods.LASTMOD > 0:
		r += "LASTMOD"
	if __mods & mods.KEY9 > 0:
		r += "K9"
	if __mods & mods.KEY10 > 0:
		r += "K10"
	if __mods & mods.KEY1 > 0:
		r += "K1"
	if __mods & mods.KEY3 > 0:
		r += "K3"
	if __mods & mods.KEY2 > 0:
		r += "K2"
	if __mods & mods.SCOREV2 > 0:
		r += "SV2(v2)"
	if __mods & mods.MIRROR > 0:
		r += "MR"
